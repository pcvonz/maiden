...
export class FennelHighlightRules extends window.ace.acequire(
  ...
  constructor() {
    super();
     this.$rules = {
        start: [{
            token: "string.quoted.double.fennel",
            regex: /"/,
            push: [{
                token: "string.quoted.double.fennel",
                regex: /"/,
                ...
                defaultToken: "string.quoted.double.fennel"
            }]
        }, {
            ...
            regex: /([[:space:]\(\)\[\]\{\}])(comment|hashfn|let|each|for|while|do|match|if|when|lambda|λ|or|and|not|require-macros|require|include|list|sym|list\?|sym\?|table\?|sequence\?|gensym|varg\?|multi-sym\?|get-scope|in-scope\?|eval-compiler|macros|macro|doto|values|partial|length|lua)([[:space:]\(\)\[\]\{\}])/
        }, {
